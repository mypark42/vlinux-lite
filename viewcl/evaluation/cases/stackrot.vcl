...
define RCUCallbackHead as Box<callback_head> [
    ...
    case ${&mt_free_rcu}:
        Box<maple_node.rcu>("maple_node": @this.next) [
            RCUCallbackHead rcu
        ]
    otherwise:
...
define RCUSegCBList as Box<rcu_segcblist> [
    ...
    case ${&mt_free_rcu}:
        Box<maple_node.rcu>("maple_node": @this.head) [
            RCUCallbackHead rcu
        ]
    otherwise:
...
current_task = TaskMM("task_current": ${per_cpu_current_task(current_cpu())})
...
diag stackrot {
    ...
    unrelated_vmas = SELECT vm_area_struct
        FROM * AS vma
        WHERE vma != 0xdeadbeef
    UPDATE unrelated_vmas WITH collapsed: true
}
